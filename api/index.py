...
def verify_firebase_token(token):
    ...
    try:
        ...
        return None
# ... (keep all your existing imports, Firebase init, verify_firebase_token, etc.) ...
...
<head>
    ...
    <style>
        ...
        #support-modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15,23,42,0.9); z-index: 1000; backdrop-filter: blur(8px); align-items: center; justify-content: center; }
...
<body>
    <div id="support-modal">
        <div class="modal-card">
            ...
            <h2 style="margin:0; font-weight:800; font-size:20px;">You've Downloaded 50 PDFs!</h2>
            <p style="font-size:14px; color:#64748b; margin:15px 0 25px;">
                Loving Pdfbirch? Consider supporting us with a small SOL donation or checking out our affiliates. Thanks for using the tool!
            </p>
            <button class="btn" onclick="openCryptoModal(); closeSupportModal()">Donate SOL</button>
            <button class="btn" onclick="closeSupportModal()" style="background:none; color:var(--primary); box-shadow:none; border:1px solid #e2e8f0; margin-top:10px;">Continue Downloading</button>
        </div>
    </div>
    <div id="crypto-modal">
        <div class="modal-card">
    ...
    <script>
        ...
        async function downloadPDF(element, token, filename) {
            ...
            try {
                ...
                // NEW: Track download count in localStorage
                let downloadCount = parseInt(localStorage.getItem('pdfbirch_download_count') || '0', 10);
                downloadCount++;
                localStorage.setItem('pdfbirch_download_count', downloadCount);
                
                // Check if over limit and show support modal
                if (downloadCount >= 50) {
                    openSupportModal();
                }
            } catch(e) {
...
# ... (keep the rest of your backend code unchanged: PREFIXES, WORDS, routes, gen_pdf_content, etc.) ...
