...
def check_limit():
    ...
    if count >= MAX_PDFS:
        return jsonify({"allowed": False, "wait_time": get_time_until_reset(email)})
...
def download():
    ...
    if not email:
        return "Unauthorized", 401
    if get_user_download_count(email) >= MAX_PDFS:
...
def gen_pdf_content():
    ...
    for _ in range(10):
        pdf.add_page()
        for _ in range(25):
            pdf.set_font(random.choice(['Arial', 'Times', 'Courier']), random.choice(['', 'B', 'I', 'BI']), random.randint(10, 14))
            line = " ".join(random.choice(WORDS) for _ in range(random.randint(10, 20))).capitalize() + "."
            pdf.multi_cell(0, 10, line)
            pdf.set_text_color(255, 255, 255)
            pdf.set_font('Arial', '', 6)
            pdf.cell(0, 5, ''.join(random.choices(string.ascii_letters + string.digits, k=15)), ln=1)
...
#         return count
#     except Exception as e:
#         print(f"Database error in get_user_download_count: {e}")
#         return 0
# def record_download(email):
...
#         return True
#     except Exception as e:
#         print(f"Database error in record_download: {e}")
#         traceback.print_exc()
#         return False
# def get_time_until_reset(email):
...
#         return 0
#     except Exception as e:
#         print(f"Database error in get_time_until_reset: {e}")
#         return 0
# # --- THE FRONTEND ---
# HTML_PAGE = """
# <!DOCTYPE html>
...
#                 if (res.status === 429) {
#                     alert('Daily quota reached! Please wait for your quota to reset.');
#                     location.reload();
#                     return;
#                 }
#                 if (res.status === 401) {
...
#             "tip": "Go to Vercel → Project Settings → Environment Variables and add DATABASE_URL"
#         }), 500

#     try:
#         # Add connection timeout and force TCP
...
#             "error": error_str,
#             "common_fixes": [
#                 "Make sure your DATABASE_URL is correct (postgres://...)",
#                 "Check if your DB allows connections from 0.0.0.0/0 (Vercel IPs)",
#                 "Try adding ?sslmode=require to your connection string",
#                 "Neon/Postgres: Go to dashboard → allow all IPs or add Vercel IPs"
#             ]
#         }), 500
# @app.route('/api/check_limit')
# def check_limit():
...
#         print(f"Checking limit for: {email}")
#         count = get_user_download_count(email)
#         print(f"Current download count: {count}/{MAX_PDFS}")
       
#         if count >= MAX_PDFS:
#             wait_time = get_time_until_reset(email)
#             print(f"Quota exceeded. Reset in {wait_time} minutes")
...
#         count = get_user_download_count(email)
#         print(f"Current download count before increment: {count}/{MAX_PDFS}")
       
#         if count >= MAX_PDFS:
#             print(f"Quota exceeded for {email}")
#             return "Quota exceeded", 429
       
#         # Record download in database
#         print(f"Recording download for {email}")
#         if not record_download(email):
#             print("Database error while recording download")
#             return "Database error - Could not record download", 500
       
#         print(f"Download recorded successfully. New count: {count + 1}/{MAX_PDFS}")
